...
public class Application {
    ...
    private synchronized void registerEventListeners() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(ExtraModInfoPayload.ID,(payload,context) -> {
            if(payload!=null && payload.extraData()!=null) {
                ...
                if(Message.equals("Refresh")){
                    onPlayerConnect(this.player);
                    server_mod.LOGGER.info("Sent and encoded fresh InventoryDataPayload successfully.");
                    ...
                    if (heldItem.getItem() instanceof BlockItem) {
                        ...
                        ServerTickEvents.END_SERVER_TICK.register(server -> {
                            ...
                            if (blockEntity instanceof ChestBlockEntity) {
                                ...
                                String blockType= blockEntity.getType().toString();
                                this.inventory.addStorage(new CustomHopper(blockname,blockType,this.inventory.getPlayerInv().getPlayerUUID(),this.inventory.getPlayerFriends(),placedPos,world));
                ...
                ServerTickEvents.END_SERVER_TICK.register(server -> {
                    ...
                    for (int i = 0; i < storageInventory.size(); i++) {
                        ...
                        if (!ItemStack.areEqual(storagePreviousState[i], currentStack)) {
                            ...
                            handleStorageInteraction((ServerPlayerEntity) player,i,storagePreviousState[i],currentStack,isOwner,isFriend,StoragePosition);
        ...
        ServerTickEvents.END_SERVER_TICK.register(server -> {
            UUID targetPlayerUUID = java.util.UUID.fromString(this.UUID);
    ...
    public void closePlayerChannels() {
        connected = false;
    }
    public synchronized Map<String, Object> PrintItemsPerCategory() {
        ...
        for (StorageInventory storage : inventory.getStorageInv()) {
            ...
            } else if (storage instanceof CustomHopper) {
                CustomHopper hopper = (CustomHopper) storage;
    ...
    private synchronized void handleStorageInteraction( ServerPlayerEntity player, int slotIndex, ItemStack previousStack, ItemStack currentStack, boolean isOwner, boolean isFriend,int[] Position) {
        String playerUUID = player.getUuidAsString();
        String action;
    ...
    public void onPlayerConnect(ServerPlayerEntity player) {
        if(connected) {
            if (player.equals(this.player)) {
                ...
                server_mod.LOGGER.info("Sent and encoded InventoryDataPayload successfully.");
            }
        } else if (!connected) {
            ServerPlayNetworking.unregisterGlobalReceiver(InventoryDataPayload.ID.id());
            ServerPlayNetworking.unregisterGlobalReceiver(ExtraModInfoPayload.ID.id());
