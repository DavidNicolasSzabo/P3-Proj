...
public class HelloController {
    ...
    public void initialize() {
        ...
        displayData(getPlayername().toString(), objectMapper);
        refreshButton.setOnAction(event -> {
            String playerName = getPlayername();
            ExtraModInfoPayload payload = new ExtraModInfoPayload("Refresh");
        ...
        addFriendButton.setOnAction(event -> {
            ...
            if (friendName != null && !friendName.isEmpty()) {
                ExtraModInfoPayload payload = new ExtraModInfoPayload("Add Friend "+friendName);
                ClientPlayNetworking.send(payload);
                cassetteField.clear();
            }
        });
        clearWarningsButton.setOnAction(event -> {
            ExtraModInfoPayload payload = new ExtraModInfoPayload("Clear Reports");
            ClientPlayNetworking.send(payload);
        });
    }
    ...
    private void displayData(String playerName, Map<String, Object> inventoryData) {
        ...
        if (storages != null) {
            ...
            contentVBox.getChildren().add(new Label("No storages available."));
        }
