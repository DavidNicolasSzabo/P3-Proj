...
@JsonSubTypes({
        ...
        @JsonSubTypes.Type(value = CustomHopper.class,name = "Hopper"),
...
public abstract class StorageInventory extends Storage {
    ...
    protected static class Slot{
        ...
        public class SlotSerializer extends JsonSerializer<Slot> {
            @Override
            public void serialize(Slot slot, JsonGenerator gen, SerializerProvider serializers) throws IOException {
                gen.writeStartObject();
                gen.writeNumberField("index", slot.index);
                gen.writeObjectField("items", slot.items);
                gen.writeStringField("stacksize", slot.stacksize.name());
                gen.writeEndObject();
            }
        }
        ...
        public class SlotDeserializer extends JsonDeserializer<Slot> {
            @Override
            public Slot deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
                JsonNode node = p.getCodec().readTree(p);
                Integer index = node.get("index").asInt();
                List<Item> items = Arrays.asList(p.getCodec().treeToValue(node.get("items"), Item[].class));
                Stacksize stacksize = Stacksize.valueOf(node.get("stacksize").asText());
                return new Slot(items, index, stacksize);
            }
        }
